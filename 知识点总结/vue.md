...
在Vue.js中，`computed`属性（计算属性）提供了一种声明式的方法来**基于其依赖数据来计算和缓存属性**值。当依赖数据发生变化时，`computed`属性会自动更新。其设计思想是提高性能，通过在依赖没有发生变化时避免不必要的计算。
...
一旦`computed`属性的**依赖数据没有变更**，**Vue会缓存上次计算的结果**，避免重复计算。这就是`computed`属性比`methods`更高效的原因。

缓存的实现是通过getter和setter来完成的。当**依赖的数据变更时**，Vue**会使缓存失效**，并在下次访问`computed`属性时重新计算。

计算属性的缓存机制是通过内部的**Watcher**实现的，一个`computed`属性**对应一个Watcher实例**。**Watcher会订阅其依赖的数据**，当**这些数据发生变化时**，会**通知Watcher刷新缓存**。
...
1. **首次访问**：
    - **第一次**读取`computed`属性时，Vue会**创建一个Watcher实例**，并**进行依赖收集**。
    - 在计算期间，所有访问到的响应式数据都会记录在Watcher的依赖列表中。

2. **依赖数据更新**：
    - 当**依赖的数据发生变化时**，这些数据的setter会**通知Watche**r，标记该计算属性为“需要重新计算”。
    - **缓存会被置为无效状态**。
...
- `vue`**组件可能会有很多个实例**，采用函数返回一个全新`data`形式，使**每个实例对象的数据不会受到其他实例对象数据的污染**
...
在Vue 2中，Vue的响应式系统是基于`Object.defineProperty`实现的，这导致了一个限制：当你动态向`data`对象**添加一个新的根级属性时**，这个**属性将不会是响应式的**。这是因为`Object.defineProperty`**只能为对象已经存在的属性定义getter和setter**，动态添加的属性不会被Vue所侦听。
...
这些**方法被重写为能够在修改数组后通知观察者（watchers）进行界面更新**。这是通过以下几步实现的：

1. **获取数组原型**：
   Vue**首先保存了原始数组原型对象**，通常为`Array.prototype`。
...
1. **对象处理**：
   - **如果目标是对象且目标属性还不存在**，使用`Object.defineProperty`为该属性**设置getter和setter**以追踪变化。
...
- **组件**是Vue应用的一个核心概念，旨在**实现视图的复用**，是一个**拥有独立视图和逻辑的一部分UI模块**。
...
- **插件**是用于为Vue应用**添加全局功能的机制**。
- 插件可以为Vue添加全局方法或资源，注入选项，添加全局指令、过滤器，以及混入等。
- 插件的目的是**扩展Vue自身，增强其功能**，以在应用中实现更复杂的场景和要求。
...
- **v-bind**绑定属性值
- **v-on**监听**input或change**事件

### Vue的双向数据绑定

Vue 的双向数据绑定是其响应式系统中的核心概念之一。它允许开发者在**视图与数据模型之间实现自动同步**，简化了数据处理的工作。Vue 使用了多种技术来实现这一功能，具体实现方式因 Vue 版本不同而有所差异。
...
- 指令处理：双向绑定主要通过指令
  ...
  - `v-model` 会**监听用户的输入事件**，当**用户输入数据时会触发数据属性的更新**。
  - **数据属性更新后**，通过**观察者机制**，**视图会自动重渲染**，并**以最新的数据值显示给用户**
...
Vue.js 的依赖收集是其响应式系统的核心机制之一，通过这个机制，Vue 能够**追踪哪些组件依赖于哪些数据**，从而在数据变化时，精准高效地更新相关的视图。理解依赖收集的过程对优化应用性能和解决更新问题至关重要。
...
2. **Dep（Dependency）**：
   ...
   - 当**属性被访问时**，**比如在模板中渲染时**，**Dep 会将当前正在计算的 Watcher 添加到自身的订阅者列表中**，这一过程**也就是依赖收集**。

3. **Watcher**：
   - **Watcher 是 Vue 中的观察者**，**组件渲染的时候会为组件实例化一个 Render Watcher**。
   - 在**属性的 getter 被触发时**，当前正在被计算的 **Watcher（通常是一个渲染 Watcher）会被记录到相应属性的 Dep 实例中**。
...
用于**在下次 DOM 更新循环结束之后执行延迟回调**。在修改数据之后立即使用这个方法，可以获取更新后的 DOM。
...
Vue 内部**使用了微任务和宏任务来实现 nextTick**。具体来说，它会优先使用 Promise（微任务），如果环境不支持 Promise，则降级到 MutationObserver（仍是微任务）。如果这两者都不支持，就会使用 setImmediate（宏任务，仅 IE 和 Node.js 支持）或 setTimeout（宏任务）。
...
执行下一次宏任务之前会执行一次 UI 渲染，等待时长比微任务要多很多。所以在能使用微任务的时候优先使用微任务，不能使用微任务的时候才使用宏任务



`Vue.nextTick` 是作为微任务实现的，这样做有以下几个原因，主要是**为了确保在当前事件循环结束之前处理数据变更后的 DOM 更新，同时不影响性能**。
...
- **开发工具支持**：Vuex **集成了 Vue DevTools**，可以**方便地进行调试和时间旅行**。
...
- **唯一性识别**：在虚拟 DOM（Virtual DOM）中的每个节点需要一个唯一的 `key`。Vue.js 使用这个 `key` 来**比较新旧节点**，从而**判断节点是否发生变化以及是否需要重新渲染**。
- **提升性能**：合适地使用 `key` 可以显著提升渲染性能，减少不必要的 DOM 操作。
- **保持组件状态**：当列表重新排序或更新时，`key` 保证了组件的状态能够正确地保留下来，而**不会因为节点位置变化导致状态丢失或混乱**。
...
1. **路由视图**：在单页应用中，用于**缓存路由组件**，避免每次进入新的路由时重新创建组件实例。
...
1. **事件修饰符**：
   ...
   - **`.self`**：只在**事件目标是当前元素自身时触发处理函数**。适用于限定事件处理范围。
...
虚拟DOM（Virtual DOM）是一个**轻量级的JavaScript对象**，它是**对真实DOM的一种抽象表示**。虚拟DOM的核心思想是：**用JavaScript对象来模拟DOM树的结构**，进行高效的DOM操作。每**当需要对界面进行更新时**，通过**对比新旧虚拟DOM的差异**，来**最小化对真实DOM的修改**。

### 为什么需要虚拟DOM

#### 1. 性能优化

真实**DOM操作（创建、更新、删除）通常是昂贵的**，尤其是在高级别的更新频繁导致浏览器重新渲染的情况下。虚拟DOM通过以下方式提升了性能：

- **Batching**: 将**多次DOM操作进行批处理**，**只生成一次批量更新**。也就是说，虚拟DOM会积累多次的状态改变，最后统一执行一次真实DOM的变更。
- **最小化DOM变更**: 通过**diff算法比较新旧虚拟DOM树的不同之处**，仅对必要的部分更新真实DOM。

#### 2. 提高跨平台能力

**虚拟DOM能够脱离浏览器平台**，**提供一种抽象层**，使得**框架可以运行在不同平台之上**。例如，Vue.js和React.js不仅可以用于Web开发，还能与其他技术结合用于移动端或是桌面端应用程序开发。
...
2. **Diff 算法**：
   ...
   - 当新的虚拟 DOM 树生成后，框架会将**新的虚拟 DOM 树与旧的虚拟 DOM 树进行比较**，找出两棵树之间的差异。
   - Vue.js 使用一种**高效的 O(n) 复杂度的 Diff 算法来比较两棵树**。
...
2. **同类节点**：**如果比较的两个节点是同类节点**（相同的标签节点），则会**深入比较其属性和子节点**。
...
Vue 的 Diff 算法**采取逐层比较的方**式，**而非逐节点递归**。这**意味着它只会比较相同层级的节点**，而不进行跨层级的比较。
...
当节点列表发生变更时（新增、删除或重新排序），Vue 使用一种高效的算法来**比较新旧两组子节点**，通常是采用**双端比较的方法（双指针法）**，以实现线性复杂度的节点对比。

### 4. 双端比较策略

#### 1. 什么是双端比较？

双端比较指的是**同时从列表的头部和尾部进行比较**。具体来说，当对两个列表进行比较时，通常有以下四种情况：
...
通过双端比较，可以**同时从头尾两个方向进行比较而不仅仅是从单一方向比较**，这样**能够更快地找到匹配的节点并进行处理**。
...
1. **更快找到匹配节点**：通过同时从头部和尾部进行比较，可以更快速地找到匹配的节点。**某些操作如节点向前移或向后移**，这种方法**能更快地检测到并处理**。
...
- **错误处理**：可以**在响应拦截器中统一处理错误，比如全局弹出错误信息提示**。
...
1. 限制访问：
   - J**avaScript 无法通过 document.cookie 访问这个 Cookie**。
...
3. **数据获取**：在**服务器渲染期间，从数据源获取数据，并将数据注入到模板中**。

4. **同构（Isomorphic/Universal）代码**：编写可以在客户端和服务器端同时运行的代码，以降低重复工作和简化逻辑控制。

5. **Hydration（复水）**：在**客户端接收从服务器返回的完全渲染的HTML页面后**，**执行再一次的React/Vue等框架的“挂载”过程**，**使其能够接管DOM的事件处理和动态更新能力**。
...
2. **用户权限存储**：用户**登录时将权限信息存储在前端**（Vuex 或 localStorage）。
...
- 在 Vue 3 中，模板编译器能够生成更优化的渲染函数，主要体现在以下几个方面：

  一、**静态提升**（Static Hoisting）

  在 Vue 2 中，每次组件重新渲染时，都会重新创建所有的虚拟 DOM 节点。而在 Vue 3 中，模板编译器会分析模板，将**静态的节点（不随数据变化而变化的部分）进行提升**。这意味着这些静态节点只在组件首次渲染时创建一次，后续的渲染中可以直接复用，从而减少了创建虚拟 DOM 的开销。
  ...
  这里的“静态文本”部分会被静态提升，在后续渲染中无需重复创建。

  二、预字符串化（Pre-stringification）

  对于包含大量静态内容的组件，Vue 3 的编译器会将这些静态部分预字符串化。这样在生成的渲染函数中，可以直接插入静态字符串，而不是通过创建多个虚拟 DOM 节点来表示静态内容。这进一步减少了运行时的开销。

  三、Patch Flag 优化

  Vue 3 为动态节点添加了“Patch Flag”（补丁标记）。这些标记可以告诉 Vue 在更新虚拟 DOM 时，具体哪些部分是动态变化的，从而使 Vue 能够更高效地进行 DOM 更新。例如，如果一个节点只有文本内容是动态的，那么在更新时，Vue 就可以只关注文本的变化，而无需检查整个节点的其他属性。

  通过这些编译优化措施，Vue 3 的模板编译器能够生成更高效的渲染函数，从而提高了应用的性能和响应速度。
...
在 Vue 3 中，**静态提升**（hoisting）是编译器优化的一部分，旨在提升应用的性能。静态提升意味着在组件的模板中，那些不参与更新的静态节点（例如纯文本节点或者没有绑定任何响应式数据的元素）会被提取到渲染函数之外，并在组件的渲染过程中被重用。这样做的好处是，静态节点在每次渲染时不需要重新创建，从而减少了不必要的性能开销。

举个例子，考虑以下 Vue 3 组件模板：
...
在这个模板中，`<p>static text</p>` 是一个静态节点，而 `<p>{{ dynamic }}</p>` 包含了一个动态绑定的变量 `dynamic`。在 Vue 3 中，静态提升会将静态节点 `static text` 提取出来，并在渲染函数之外创建一个对应的常量，如下所示：

javascript

```javascript
const _hoisted_1 = /*#__PURE__*/createStaticVNode("<div><p>static text</p></div>", 1)
```

然后，渲染函数会变得更简洁，它直接使用 `_hoisted_1` 而不是每次都创建一个新的 `<p>` 节点：
...
function render(_ctx, _cache) {
  return (_openBlock(), _createBlock("div", null, [
    _hoisted_1,
    createVNode("p", null, _toDisplayString(_ctx.dynamic), 1 /* TEXT */)
  ]))
}
```

通过这种方式，静态节点在组件的整个生命周期中只会被创建一次，在后续的渲染中可以直接复用。这显著减少了虚拟 DOM 的创建和比对开销，提高了应用的性能。

## Proxy API ------ Object.defineProperty API 

**它只能用于已知的属性名**，无法动态处理新增属性, 这就是为什么 Vue 3 引入了 **Proxy**。
...
Vuex 是 Vue.js 的状态管理模式和库，它采用集中式存储管理应用的所有组件的状态，并以相应的规则保证状态以一种可预测的方式发生变化。在 Vuex 中，`commit` 方法是用来提交 `mutation` 的，它设计为同步函数，主要有以下几个原因：

### 1. **保持状态变更的可预测性**
使用 Vuex 时，所有的状态变更都应该通过 `mutation` 来进行。`mutation` 被设计为同步函数，这样可以保证状态变更是有序和可预测的。如果 `mutation` 是异步的，那么状态的变更可能会变得难以追踪和预测，因为异步操作可能随时更改状态，导致状态变化不可预测。

### 2. **便于调试**
同步的 `mutation` 使得状态变更变得容易追踪和调试。开发者可以明确知道状态是在哪个时间点被改变的，因为每个状态变化都对应一个明确的 `mutation`。如果 `mutation` 是异步的，那么状态变化可能和异步操作交织在一起，使得调试变得更加困难。

### 3. **符合 Vue 的响应式原理**
Vue 的响应式系统依赖于能够追踪状态变化的能力。当状态以同步的方式变更时，Vue 可以追踪这些变化并相应地更新 DOM。如果 `mutation` 是异步的，那么状态变化就不是同步发生的，这可能会导致 Vue 的响应式系统无法正确地追踪状态变化，进而导致 UI 不同步。

### 4. **简化状态管理**
将 `mutation` 保持为同步操作，可以简化状态管理的逻辑。开发者不需要考虑异步操作中可能出现的竞态条件、回调地狱等问题。这使得状态管理更加直观和容易理解。

### 5. **性能考量**
虽然性能不是主要考虑因素，但同步 `mutation` 可以减少执行状态变更所需的时间和资源，因为不需要处理异步操作的开销。

### 总结
Vuex 的 `commit` 方法不是异步的，是为了保持状态变更的可预测性、便于调试、符合 Vue 的响应式原理、简化状态管理，并可能带来性能上的好处。如果需要执行异步操作，可以在 `action` 中进行异步操作，然后通过 `commit` 来同步地提交 `mutation`。这种方式既保持了状态管理的清晰和可预测，又能处理异步逻辑。
...
1. **`页面刷新`的本质**：当浏览器页面执行刷新（例如按下F5或点击浏览器刷新按钮）时，**整个页面会进行重新加载**。此过程**实际上是在结束当前页面的所有活动，包括JavaScript的执行**，然后重新加载资源并重新运行JavaScript代码。这个过程并不会触发组件的正常生命周期销毁钩子，因为这是一种`“硬”终止，不会走正常的组件卸载流程`。

2. **设计问题**：组件销毁钩子主要用于处理组件在被正常替换或移除时的清理工作，如移除事件监听器、停止定时器等。当**进行页面刷新时，这些操作通常由浏览器自动处理**（如清除内存中的变量等），因此`不需要组件自身显式触发销毁钩子`。
...
总之，页面刷新不会触发组件的正常销毁钩子，因为**页面刷新意味着整个页面的销毁和重新加载**，**这是一个浏览器级的操作**，不是单个组件或应用的生命周期管理。如果需要在`页面刷新时进行特定操作`，应该考虑使用**浏览器提供的事件**如 `beforeunload`。
...
在Vue 3中，引入了一个名为 `setup` 的新函数，它是使用组合式API时的一个主要功能。`setup` 函数是**组件的入口点，它在组件创建之前执行**，允许你定义组件的响应式状态、计算属性、侦听器和其他函数。这标志着Vue对于更具函数风格编程的支持，旨在促进更大程度的代码复用和更清晰的逻辑分离。
...
- Vue 编译器在编译模板时会自动解包 ref
- 在模板编译过程中，会检测到 ref 对象并自动调用其 value 属性
...
Vue 3 通过 `Proxy` 和 `WeakMap` 组合实现响应式数据跟踪。`WeakMap` 在响应式系统中用于存储对象与其依赖之间的映射关系，确保当对象不再被引用时能够自动清理，避免内存泄漏。

具体来说，Vue 3 的响应式机制大致流程如下：

1. **Proxy 拦截**：Vue 3 使用 `Proxy` 代理对象的读写操作。当你访问对象的某个属性时，`Proxy` 会拦截并记录下该属性与依赖的关系（例如某个组件或计算属性），这样当该属性发生变化时，Vue 就能通知相应的依赖去更新。

2. **WeakMap 存储依赖关系**：Vue 3 使用一个全局的 `WeakMap` 来存储对象及其依赖映射。`WeakMap` 的键是响应式的对象，值则是该对象依赖的 `Map`，每个 `Map` 会进一步存储对象各个属性与依赖之间的关系。

   ```javascript
   const reactiveMap = new WeakMap(); // Vue 3 响应式的 WeakMap
   ```

   这样，当响应式对象不再被使用时（即没有强引用存在时），它会被垃圾回收机制清理掉，同时 `WeakMap` 中的相应条目也会被自动移除，不会导致内存泄漏。

3. **依赖收集**：当访问对象的某个属性时，Vue 3 会通过 `WeakMap` 查找当前对象是否已被代理过，如果没有则创建一个新的 `Map` 来存储该对象的属性依赖。每个属性都可以有多个依赖（如多个组件、计算属性），Vue 会将这些依赖与属性一一对应。

4. **依赖触发**：当修改响应式对象的某个属性时，Vue 3 通过 `WeakMap` 找到该属性的依赖，并通知依赖进行更新。
