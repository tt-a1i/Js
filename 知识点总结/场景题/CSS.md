...
1. **CSS Modules**：
   - **原理**：将CSS**类名局部化，通过生成唯一的哈希值**来避免样式冲突。
   - **实现**：常用于Webpack中，使用特定的文件扩展名（如`.module.css`）。
   - **优点**：确保类名在全局范围内唯一，避免样式泄漏。

2. **CSS-in-JS**：
   - **原理**：将**CSS样式直接写在JavaScript中**，**样式作用域限制在组件内部**。
   - **实现**：常用的库有Styled Components、Emotion。
   - **优点**：样式与组件逻辑紧密结合，方便管理和维护。

3. **预处理器模块化**：
   - **Less和Sass**：
     - **实现**：使用`@import`语句引入其他文件，实现模块化组织。
     - **优点**：支持变量、 mixins等功能，增强样式逻辑的复用。

4. **Atomic CSS（Utility-first CSS）**：
   - **原理**：提供大量的原子级别的类名，组合使用构建页面样式。
   - **实现**：如Tailwind CSS。
   - **优点**：类名职责单一，减少冗余，提高复用性。

5. **CSS Variables (Custom Properties)**：
   - **原理**：定义可重用的变量，提高样式的一致性和维护性。
   - **实现**：在CSS中直接定义`--variable-name`，并在其他地方引用。
   - **优点**：动态修改样式，增强灵活性。

6. **Scoped CSS**：
   - **原理**：在特定范围内隔离样式，如Vue框架中的`<style scoped>`。
   - **实现**：通过编译器自动添加作用域标识。
   - **优点**：样式仅作用于当前组件，避免全局污染。

7. **ES6模块系统**：
   - **原理**：将CSS文件作为模块导入到JavaScript文件中。
   - **实现**：结合Webpack等工具，实现CSS的按需加载。
   - **优点**：模块化管理和按需加载，优化资源加载。

8. **CSS Namespaces**：
   - **原理**：通过**类名前缀实现模块化**，避免样式冲突。
   - **实现**：手动添加前缀。
   - **优点**：简单直接，但现代工具支持较少。

## 在使用scoped之后，怎么在父元素中覆盖子元素的样式

#### 1. 使用深度选择器（Deep Selector）

Vue.js 提供了深度选择器 `>>>` 或 `/deep/` 来穿透 `scoped` 样式，使得父组件的样式可以影响到子组件。
...
1. **性能**：将 `<style>` 标签放在 `<head>` 中有助于**提高页面的加载性能**。这是因为浏览器在解析HTML文档时，会先构建DOM树和CSSOM（CSS对象模型）树，然后根据这两个树构建渲染树。如果样式信息提前加载，浏览器可以更快地完成渲染树的构建，进而加快页面的渲染速度。

2. **渲染行为**：当 `<style>` 标签位于 `<head>` 中时，浏览器会在解析HTML文档的同时解析样式信息，这样可以**确保在页面内容渲染之前，样式已经被应用**。这避免了页面先以默认样式显示，再重新应用样式导致的闪烁现象（FOUC，Flash of Unstyled Content）。

### `<style>` 放在 `<body>` 中

1. **维护性**：将 `<style>` 标签放在 `<body>` 中，尤其是在文档的不同部分分散放置，可能会导致样式难以管理和维护。当文档内容较多时，寻找和修改特定的样式信息会变得困难。

2. **性能**：如果 `<style>` 标签位于 `<body>` 中，浏览器在解析到该标签时会触发重绘或重排（reflow），因为此时页面的部分内容可能已经渲染。这会导致额外的性能开销，尤其是在样式频繁变化的情况下，可能严重影响页面的加载速度和响应性能。

3. **渲染行为**：将 `<style>` 标签放在 `<body>` 中可能导致FOUC现象，即页面先以默认样式显示，然后在样式加载后重新渲染。这不仅影响用户体验，还可能造成页面布局的不稳定。
...
在 CSS 中，`z-index` 属性用于控制元素的堆叠顺序（即哪个元素在上面，哪个元素在下面）。当多个元素的 `z-index` 值不同时，`z-index` 值较大的元素会显示在 `z-index` 值较小的元素之上。然而，当子元素距离根元素的距离不一样时，`z-index` 的优先级判断会变得更加复杂。

### 堆叠上下文（Stacking Context）

在理解 `z-index` 的优先级之前，需要了解堆叠上下文（Stacking Context）的概念。堆叠上下文是一个三维的概念，元素在 Z 轴上的顺序由以下因素决定：

1. **根元素（HTML）**：根元素形成一个根堆叠上下文。
2. **`z-index` 值不为 `auto` 的定位元素**：具有 `position` 属性（如 `relative`、`absolute`、`fixed`、`sticky`）且 `z-index` 值不为 `auto` 的元素会形成新的堆叠上下文。
3. **其他属性**：如 `opacity`、`transform`、`filter`、`will-change` 等属性也会创建新的堆叠上下文。

### 优先级判断

当子元素距离根元素的距离不一样时，`z-index` 的优先级判断遵循以下规则：

1. **同一堆叠上下文内的元素**：
   - 在同一堆叠上下文内，`z-index` 值较大的元素会显示在 `z-index` 值较小的元素之上。
   - 如果 `z-index` 值相同，则根据 HTML 文档中的顺序（后出现的元素在上面）。

2. **不同堆叠上下文内的元素**：
   - 不同堆叠上下文内的元素的 `z-index` 值是相互独立的。即使某个子元素的 `z-index` 值很大，但如果它所在的堆叠上下文的 `z-index` 值较小，它仍然可能被其他堆叠上下文中的元素覆盖。
   - 堆叠上下文的顺序由其父元素的 `z-index` 值决定。父元素的 `z-index` 值较大的堆叠上下文会显示在父元素 `z-index` 值较小的堆叠上下文之上。

### 示例

以下是一个示例，展示了不同堆叠上下文内的元素的优先级判断：

```html
<div style="position: relative; z-index: 1;">
  <div style="position: absolute; z-index: 10;">Box 1</div>
</div>

<div style="position: relative; z-index: 2;">
  <div style="position: absolute; z-index: 5;">Box 2</div>
</div>
```

在这个示例中：

- `Box 1` 和 `Box 2` 分别位于两个不同的堆叠上下文中。
- `Box 1` 的 `z-index` 值为 10，`Box 2` 的 `z-index` 值为 5。
- 由于 `Box 2` 所在的堆叠上下文的 `z-index` 值为 2，而 `Box 1` 所在的堆叠上下文的 `z-index` 值为 1，因此 `Box 2` 会显示在 `Box 1` 之上，即使 `Box 1` 的 `z-index` 值较大。

### 总结

当子元素距离根元素的距离不一样时，`z-index` 的优先级判断需要考虑堆叠上下文的影响。同一堆叠上下文内的元素的 `z-index` 值决定它们的堆叠顺序，而不同堆叠上下文内的元素的堆叠顺序由其父元素的 `z-index` 值决定。通过理解堆叠上下文的概念，你可以更好地控制元素的堆叠顺序。
