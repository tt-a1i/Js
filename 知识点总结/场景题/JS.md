...
History模式需要服务器支持的原因主要在于其工作原理和对URL路径的处理方式。在History模式下，URL中的路径部分被用来管理路由，**当用户刷新页面或直接访问某个路由时，浏览器会向服务器发送请求**。如果服务器没有正确配置，这些请求可能会导致404错误，因为服务器上可能不存在对应的静态文件来响应这些请求。

具体来说，History模式利用HTML5的History API来实现页面跳转，这意味着URL的变化不会引起页面的刷新，而是通过JavaScript动态更新浏览器地址栏的URL。然而，当用户手动刷新页面或直接输入一个非根URL时，浏览器会向服务器发送一个请求，而服务器如果没有配置好，将无法识别这些请求并返回正确的页面内容，从而导致404错误。

为了防止这种情况发生，服务器需要进行相应的配置，以确保在任何路径下都能返回前端应用的入口页面（通常是index.html）。这样，前端路由就可以接管这些请求，并根据当前的URL路径加载相应的组件，从而避免404错误。

此外，使用History模式还有助于改善用户体验和SEO效果，因为URL中不再包含#符号，看起来更加美观且符合用户的浏览习惯。但这也意味着开发者需要额外关注服务器配置，以确保History模式能够正常工作。

## history模式为什么有利于SEO

![image-20241124193446603](./assets/image-20241124193446603.png)

1. **URL结构更符合传统形式**：在History模式下，URL不再包含哈希符号（#），而是使用真实的路径形式，例如：http://example.com/home 。这种URL结构更加直观、语义化，能够更好地反映页面的内容和层级关系，从而有助于搜索引擎理解和索引页面内容。
2. **搜索引擎更容易抓取和索引**：由于History模式的URL路径部分直接与服务器交互，搜索引擎可以更容易地抓取和索引这些页面。相比之下，Hash模式下的URL由于包含“#”符号，搜索引擎通常不会抓取其后的部分，因此对SEO不太友好。
...
1. **使用 iframe + postMessage**：
   - 在不同域名的页面中嵌入一个隐藏的 iframe，并通过 postMessage 方法进行跨域通信。父页面可以向 iframe 发送请求，iframe 接收到请求后，将 localStorage 中的数据通过 postMessage 返回给父页面。
2. **使用 postMessage API**：
   - 利用不同窗口（包括不同域名和协议）之间的 postMessage API 进行安全的消息传递，将数据存储到 localStorage 中。
3. **使用服务器端中转**：
   - 如果拥有控制权的服务器，可以将数据发送到服务器端存储在自己的 localStorage 中，需要时再从服务器端获取数据。
4. **使用第三方库**：
