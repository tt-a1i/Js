...
在 JavaScript 中，使用 `new` 创建对象和直接声明变量有很大的区别，主要体现在**内存分配**、**原型继承**、**构造函数调用**等方面。以下是具体的对比：

### 1. `new` 的用法

`new` 是用来创建对象的关键字，通过调用构造函数来创建对象实例。它会经历以下步骤：

- **创建一个新的空对象**。
- **将该对象的原型指向构造函数的 `prototype`**，从而继承构造函数的属性和方法。
- **调用构造函数**，并将 `this` 绑定到新创建的对象。
- **返回新对象**，如果构造函数没有显式返回对象，则返回新创建的对象。
...
在这个例子中，`person` 是通过 `new` 关键字创建的对象，继承了 `Person` 构造函数的属性和方法。

### 2. 直接声明变量的用法

直接声明变量不涉及构造函数调用和原型设置。使用 `const`、`let` 或 `var` 声明的变量，通常用于存储基本数据类型（如 `string`、`number`）或者对象字面量。
...
这里 `person` 是一个普通对象，没有任何原型关系或构造函数调用，也没有 `Person` 的属性或方法。

### 区别总结

| 特性               | 使用 `new`                                | 直接声明变量                       |
| ------------------ | ----------------------------------------- | ---------------------------------- |
| **适用场景**       | 创建对象实例，使用构造函数                | 创建基本变量或对象字面量           |
| **原型继承**       | 新对象继承构造函数的 `prototype`          | 不涉及继承                         |
| **构造函数调用**   | 调用构造函数，`this` 绑定新对象           | 不调用构造函数                     |
| **内存分配**       | 为每个实例分配独立的内存空间              | 基本类型直接存储，引用类型共享引用 |
| **属性和方法共享** | 实例共享构造函数的 `prototype` 属性和方法 | 无共享机制，方法需显式声明         |

### 何时使用 `new` 和直接声明
- 使用 `new`：适合需要创建多个实例的情况，可以利用原型链共享属性和方法。
- 直接声明：适合声明简单变量或对象，不需要额外的原型继承和构造函数调用。
