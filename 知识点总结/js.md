...
闭包占用内存，因为它**保留了创建时的作用域上下文**。大量使用闭包可能会**导致内存占用过多**，因此在性能敏感的应用中应谨慎使用。
...
css属性 **pointer-events: none,**加了这个属性的元素**不会成为event.target**,保证非目标元素不会为冒泡的target
...
3. **将构造函数的 `this` 绑定到新创建的对象**：
   ...
   - 这意味着在执行构造函数内的代码时，所有对`this`的引用实际上都是指向这个新创建的对象
   - `new`操作符会使用提供的参数（如果有）来调用构造函数。此时，构造函数内部的代码开始执行，它可以在新对象上添加属性和方法
...
在浏览器里，每**当一个事件发生并且有一个事件监听器绑定在该事件上时**，一个**消息就会被添加进消息队列**。**如果没有事件监听器**，这个**事件将会丢失**。**所以当一个带有`点击事件`处理器的元素被点击时**，就会像其他事件一样产生一个类似的消息。
...
事件循环的主要作用是协调浏览器的异步操作，比**如用户的输入**、网络请求、**计时器等**。这些操作大部分是通过**宏任务**（macrotask）实现的，比如：
...
## 同步任务,宏任务,微任务与页面渲染之间的关系

### 执行顺序的总体流程

1. 执行同步任务，直到调用栈为空。
2. 清空微任务队列（即所有 `Promise.then`、`MutationObserver` 等微任务）。
3. 进行页面渲染。
4. 执行下一个宏任务。
5. 重复以上步骤。
...
- **同步任务**会立即执行，不会进入任务队列，而是直接在主线程上依次执行。
- 典型的同步任务包括变量声明、函数调用、`console.log` 等。
- **执行时机**：所有同步任务会在微任务、宏任务之前依次执行。

### 2. 微任务

- **微任务**是一类在当前事件循环完成后、进入下一次事件循环之前执行的任务。
- 常见的微任务包括 `Promise.then`、`MutationObserver`、`queueMicrotask` 等。
- **执行时机**：在每次同步任务完成后，主线程会清空微任务队列，确保所有微任务在进入下一个事件循环前执行完毕。

### 3. 宏任务

- **宏任务**是相对较低优先级的任务，它会在每次事件循环结束后执行。
- 常见的宏任务包括 `setTimeout`、`setInterval`、`setImmediate`、事件监听等。
- **执行时机**：每轮事件循环的末尾，浏览器会在所有微任务完成后执行一个宏任务。

### 4. 页面渲染

- 页面渲染是浏览器绘制页面的过程，通常发生在每次事件循环的结束和下一次事件循环开始之间。
- 页面渲染的典型频率是每秒 60 次（即每 16.67 毫秒刷新一次），但可能受限于系统性能或页面复杂度。
- **执行时机**：页面渲染发生在微任务清空之后、宏任务之前。
...
因此，`requestAnimationFrame` 不属于典型的宏任务或微任务，而是由浏览器优化控制，以便在合适的时间触发，从而确保动画和绘制的流畅性。
...
1. **遍历对象属性**：适用于遍历对象的键名。
2. **遍历数组索引**：也可以用于遍历数组，但**遍历的是数组的索引**。
...
`for...of` 语句是用来**遍历可迭代对象**（包括**数组、字符串、Set、Map** 等等）的值。`for...of` 不能直接用于普通对象（因为普通对象不可迭代），但对象的属性可以通过其他方式（如使用 `Object.keys()`）转为可迭代对象，再使用 `for...of`。
...
Vue Router 是 Vue.js 官方的路由管理器，它允许你在单页应用（SPA）中实现页面之间的导航。Vue Router 提供了两种路由模式：**Hash 模式**和**History 模式**。这两种模式在实现路由的方式和浏览器行为上有所不同。

### 1. Hash 模式

**定义**：
Hash 模式使用 URL 中的 `#` 符号来模拟完整的 URL，当 `#` 后面的内容发生变化时，浏览器不会向服务器发送请求，而是通过监听 `hashchange` 事件来更新页面内容。
...
**特点**：
- **简单易用**：Hash 模式实现简单，不需要服务器端的支持。
- **兼容性好**：由于 `#` 符号后面的内容不会发送到服务器，因此兼容性非常好，适用于所有浏览器。
- **SEO 不友好**：由于 URL 中包含 `#` 符号，搜索引擎可能无法正确解析这些 URL，因此对 SEO 不太友好。

### 2. History 模式

**定义**：
History 模式利用 HTML5 的 `history.pushState` API 来实现 URL 的导航，而不会重新加载页面。这种模式下，URL 看起来和传统的 URL 一样，没有 `#` 符号。
...
**特点**：
- **美观**：URL 看起来更加美观，没有 `#` 符号。
- **SEO 友好**：由于 URL 和传统 URL 一致，对 SEO 更加友好。
- **需要服务器配置**：由于 URL 中没有 `#` 符号，当用户直接访问这些 URL 时，服务器需要正确处理这些请求，否则会返回 404 错误。因此，需要对服务器进行配置，以确保所有路由都指向同一个 HTML 文件（通常是 `index.html`）。



- **Hash 模式**：简单易用，兼容性好，但对 SEO 不友好。适用于不需要 SEO 支持的场景。
- **History 模式**：URL 美观，对 SEO 友好，但需要服务器配置。适用于需要 SEO 支持的场景。
...
服务端渲染的 HTML **无需等到所有的 JavaScript 都下载并执行完成之后才显示**，所以你的用户将会更快地看到完整渲染的页面。除此之外，数据获取过程在首次访问时在服务端完成，相比于从客户端获取，可能有**更快的数据库连接**。
...
这个设计是为了避免单线程环境中的“**任务饥饿**”问题。JavaScript 的事件循环会处理执行栈、微任务（如 Promise）和宏任务（如 setTimeout）。当**宏任务的层级超过一定深度时**，JavaScript **引擎会引入最小延迟**，以**确保其他任务有机会执行**，**从而保持页面的响应性**。这样**可以让界面更新和用户交互更加流畅**。
...
| ---------------- | --------------------------------------------------------- | ------------------------------------------------ |
| **数据结构类型** | **有序集合**，按照索引（下标）存储数据                    | **键值对（key-value pair）集合**                 |
| **键的类型**     | 键是**数字索引**，从 0 开始递增                           | 键可以是**字符串或 Symbol**                      |
| **数据访问方式** | 使用**索引**（如 `arr[0]`）访问元素                       | 使用**键**（如 `obj['name']` 或 `obj.name`）访问 |
| **适用场景**     | 用于存储**有序列表**（如数组、队列、栈）                  | 用于存储**非线性数据**（如对象的属性和方法）     |
| **长度**         | 有**length**属性，可以动态变化                            | 没有长度限制，可以随时添加/删除键值对            |
| **遍历方式**     | `for`，`forEach`，`map`，`filter` 等                      | `for...in`，`Object.keys()`，`Object.entries()`  |
| **性能特点**     | 插入和删除元素比较慢，适合读取和遍历                      | 查找和插入性能较好，适合存储结构化数据           |
| **内置方法**     | 有**丰富的数组操作方法**，如 `push()`，`pop()`，`slice()` | 通过对象字面量或 `Object` 相关方法管理           |

---

### **数组的特点与用法**  
数组是一种**有序数据集合**，常用于存储一组相同或不同类型的元素。  
#### **适用场景**
- 存储**列表、队列、栈**等有序数据。
- 当你需要频繁**遍历或按顺序访问**数据时，使用数组更合适。

---

### **对象的特点与用法**  
对象是一种**无序的键值对集合**，常用于描述一个实体的属性和方法。

#### **适用场景**
- 描述**实体的属性和行为**（如用户、商品、配置项等）。
- 当数据没有顺序要求，且需要通过键值访问时，使用对象更合适。

---

### **数组与对象的相互关系**
- **数组也是一种对象**：在 JavaScript 中，数组是对象的子类型（`typeof [] === 'object'`）。
- **数组的键是数字索引**：**数组内部的索引其实也是对象的键，但它们是按顺序递增的数字**。
- **对象中可以嵌套数组**，数组中也可以包含对象：
  
  ```javascript
  let objWithArray = { data: [1, 2, 3] };
  let arrayWithObjects = [{ id: 1 }, { id: 2 }];
  ```

---

### **总结：何时用数组，何时用对象？**
- **使用数组**：当你需要**按顺序存储和遍历数据**时，数组是最佳选择，比如列表、队列、栈。
- **使用对象**：当你需要存储**键值对结构的复杂数据**（如描述一个人、一个商品）时，对象更合适。  

## js中new和直接声明变量的区别

<img src="./assets/image-20241112103645169.png" alt="image-20241112103645169" style="zoom:67%;" />

1. **创建对象实例**：
   - 使用`new`关键字时，会调用构造函数来创建一个新的对象实例。这个新对象实例会继承构造函数的属性和方法。
   - 直接声明变量时，不会创建新的对象实例，而是**直接赋值给变量**。例如，`var a = "abc";` 直接将字符串"abc"赋值给变量a。
2. **类型和内存管理**：
   - 使用`new`关键字创建的对象是**对象类型**，而直接声明的变量可以是基本数据类型（如字符串、数字、布尔值等）。
   - 使用`new`关键字创建的对象**在内存中占用更多的空间**，因为它们**包含更多的属性和方法**。
3. **变量类型**：
   - 直接声明变量时，JavaScript会**根据赋值自动推断变量的类型**。例如，`var a = "123";` 和 `var b = 123;` 是不同的类型。
   - 使用`new`关键字创建的对象类型是明确的，例如 `var str = new String("123");` 明确表示这是一个字符串对象, 如果传入123会出现类型报错。
4. **性能考虑**：
   - 直接声明变量通常比使用`new`关键字更高效，因为**直接赋值不需要创建新的对象实例**。
   - 使用`new`关键字创建对象可能会导致性能开销，尤其是在频繁创建对象的情况下。
