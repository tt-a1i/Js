...
<header>：定义页面或者文章的页眉，通常包含导航链接、网站标志等。
<nav>：定义导航链接的部分，用于包含网站的主要导航菜单或者其他导航元素。
<main>：定义文档的主要内容，用于包含页面的核心内容部分。
<section>：定义文档的一个区域或者一个章节，通常具有相关的主题或者内容。
<article>：定义一个独立的内容块，如一篇文章、一篇博客帖子等。
<aside>：定义与页面内容相关但不是主要内容的部分，如侧边栏、广告等。
<footer>：定义页面或者文章的页脚，通常包含版权信息、联系方式等。
<figure>：定义一个独立的内容块，通常与图片、图表、视频等相关联，并可包含一个标题。
<figcaption>：定义 <figure> 元素中的标题，描述与之相关的内容。
<time>：定义时间或者日期，可用于表示发布日期、事件日期等。
<address>：定义联系信息，通常用于包含作者或者拥有者的联系信息。
这些标签有助于构建更具语义化的页面结构，提高页面的可读性和可访问性，同时也有利于搜索引擎优化。
### 304 过程
