...
1. 多路复用 (Multiplexing)
   - 允许同时通过单一的 TCP 连接发送多重请求-响应消息
   - 解决了 HTTP/1.1 的队头阻塞问题
   - 减少了连接建立的开销
2. 二进制分帧层 (Binary Framing Layer)
   - 将信息分割为更小的消息和帧，并采用二进制格式编码
   - 提高了数据传输的效率和可靠性
...
6. 流量控制 (Flow Control)
   - 针对每个流的流量控制
   - 防止发送方overwhelm接收方
7. 一个连接多个请求 (One Connection Per Origin)
   - 所有到同一域名的请求都在同一连接上完成
   - 减少了连接建立的开销
8. 安全性改进
   - 虽然 TLS 不是强制的，但大多数实现都要求使用 TLS
   - 更好地支持了加密通信
9. 协议协商机制 (Protocol Negotiation)
   - 允许客户端和服务器协商使用 HTTP/2 或回退到 HTTP/1.1
10. 重置流 (Stream Reset)
    - 允许立即终止一个流，而不影响 TCP 连接
11. 帧类型扩展性
    - 允许未来添加新的帧类型，提高了协议的可扩展性
