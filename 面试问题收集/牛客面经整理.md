...
2. 通过 JavaScript 获取伪元素：
   ...
   - 但可以通过 `getComputedStyle() `方法`间接访问伪元素的样式`。
...
6. `滚动节流`
   ...
   - 这可以减少高频滚动时的计算压力。
7. 异步渲染
   - 将`渲染过程分解为多个小任务`，使用 requestAnimationFrame 进行调度。
   - 这可以防止长时间的渲染阻塞主线程。
8. 使用虚拟化库
...
http本身是无状态的,所以需要用cookie,session这种来保存用户的状态

对于1台服务器,每次发送session来确定状态没有问题,但是如果是负载均衡的多台服务器,其他服务器上没有对应的session,用户要重新登录,这样就不可行
...
`JWT`由三部分`组成`，分别是`头部`（Header）、`载荷`（Payload）和`签名`（Signature），`头部`包含有关JWT的`元数据和算法信息`，`载荷`包含实际的`用户数据`，例如`用户ID`、`角色`等，`签名`用于`验证JWT的完整性`和真实性。通过签名保证安全,如果修改了jwt的内容,签名就对不上了

### JWT 的优点

- **无状态:** 服务器不需要存储用户会话状态，减轻了服务器的负担，提高了系统的可扩展性。
- **分布式系统友好:** JWT 可以跨多个服务和域名进行传递，非常适合微服务架构。
- **可读性强:** JWT 的结构清晰，易于理解和调试。
- **安全性高:** 使用数字签名可以防止数据篡改，保证数据的完整性。
- **自定义数据:** 可以自定义 JWT 中包含的声明，灵活地控制授权。

### JWT 的缺点

- **Token 泄露风险:** 一旦 Token 被泄露，攻击者可以冒充用户进行操作。
- **Token 撤销困难:** JWT 一旦生成，很难在服务器端撤销，通常需要设置较短的过期时间。
- **Token 大小:** JWT 中包含了较多的信息，可能会导致 Token 的大小增加，影响传输性能。
- **单点登录存在挑战:** 在单点登录场景下，当用户密码变更时，需要及时更新所有系统中的 Token，这可能比较复杂。



`JWT和token的区别`主要`体现在是否需要查询数据库`。对`Token`来说，`服务端验证客户端发送过来的 Token 时，还需要查询数据库获取用户信息`，然后`验证 Token 是否有效`。而`JWT则不需要`，因为`用户的信息及加密信息、过期时间，都存储在JWT里`，服务端只需要`使用密钥解密进行校验`即可，不需要查询或者减少查询数据库。
...
7. SameSite: 用来`防止 CSRF 攻击和用户追踪`。
   ...
   1. **Strict:** 浏览器只会在同站请求中发送 Cookie，任何跨站请求都不会携带 Cookie，这能有效防止 CSRF 攻击。

      **Lax:** 除了 Strict 的限制外，在安全的跨站请求（比如通过 GET 方法提交的表单）中也会发送 Cookie。

      **None:** Cookie 无条件发送，但需要配合 Secure 属性使用，否则浏览器可能会拒绝发送。
