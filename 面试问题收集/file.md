...
4. HTTP/2 (2015)
   - 使用二进制格式传输数据，而不是文本格式
   - 多路复用：允许同时通过单一的 TCP 连接发送多重请求-响应消息
   - 头部压缩：减少了传输数据的大小
...
1. 键的类型：
   ...
   - Map：键可以是任何类型，包括对象、函数等。
2. 键的顺序：
   - Object：键的顺序不保证（虽然现代浏览器通常会保持插入顺序）。
   - Map：保证按插入顺序迭代键值对。
3. 大小：
...
4. 迭代：
   ...
   - Map：可以直接迭代，有 keys()、values() 和 entries() 方法。
5. 性能：
   - Object：在少量键值对时表现良好。
   - Map：在频繁增删键值对时性能更好，尤其是处理大量数据时。
6. JSON：
...
原型链是 JavaScript 中实现继承的一种机制。它是通过对象之间的链接（称为原型）来实现的。以下是关于原型链的一些重要概念和特点：
...
2. **proto** 属性
   ...
   - **proto** 指向该对象的原型对象。
3. 原型链
   - 当访问一个对象的属性时，如果对象本身没有这个属性，JavaScript 会沿着原型链向上查找。
   - 这个查找过程会一直持续到找到属性或到达原型链的顶端（通常是 Object.prototype）。
4. Object.prototype
